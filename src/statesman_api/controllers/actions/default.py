...
def execute(org_id: str, user_id: str, args: list) -> list:
    ...
    if len(args) != 2:
        data = build_error_data("Usage: `default <name> <value>`.")
        return data, True
    ...
    if collection is None:
        data = build_error_data("Unable to set item default; no current collection is set.\nTry one of these:") + list_collections(user_id, org_id)
        return data, True
    ...
    if item is None:
        ...
        if not check_item_permission(user, item, model_constants.PERMISSION_WRITE):
            data = build_error_data("Unable to set default on this item; you do not have permission to write to it.")
            return data, True
        ...
        data = build_message_data(f"Updated item *{name}* with default value *{value}*.")
