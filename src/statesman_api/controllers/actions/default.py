...
def execute(org_id: str, user_id: str, args: list) -> list:
    ...
    if len(args) != 2:
        blocks = build_error_data("Usage: `default <name> <value>`.")
        return blocks
    ...
    if collection is None:
        blocks = build_error_data("Unable to set item default; no current collection is set.\nTry one of these:") + list_collections(user_id, org_id)
        return blocks
    ...
    if item is None:
        blocks = build_error_data(f"No item exists for name *{name}*.")
    else:
        if not check_item_permission(user, item, model_constants.PERMISSION_WRITE):
            blocks = build_error_data("Unable to set default on this item; you do not have permission to write to it.")
            return blocks
        ...
        blocks = build_message_data(f"Updated item *{name}* with default value *{value}*.")

    return blocks, False


def help_info():
    return ("default", "Set Default", "Set a item's default value: `set <name> <value>`", None)
