...
class BaseConfig(object):
    ...
    SECRET_KEY = os.environ.get(constants.SECRET_KEY, hashlib.sha256(f"{random.random()}").hexdigest())
    CSRF_TOKEN = os.environ.get(constants.CSRF_TOKEN, hashlib.sha256(f"{random.random()}").hexdigest())
